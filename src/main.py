...
def main() -> None:
    ...
    manager, nodes, curr_edges, routes, vehicles, traffic_lights = load_preset(preset_name)
    intersection_points = get_intersections(routes)
    run_simulation(vehicles, nodes, curr_edges, routes, intersection_points, manager, traffic_lights)

def load_preset(file_path: str) -> tuple[Manager, list[Node], list[Edge], list[Route], list[Vehicle], list[TrafficLight]]:
    with open(file_path, 'r') as file:
        presets = json.load(file)

    nodes, curr_edges, routes, vehicles, traffic_lights = [], [], [], [], []
    ...
    return manager, nodes, curr_edges, routes, vehicles, traffic_lights
